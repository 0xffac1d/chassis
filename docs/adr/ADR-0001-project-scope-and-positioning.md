...
5. **Self-governed surface claims.** The root contract claim `chassis.jsonrpc-not-mcp` is enforced by this scope boundary: the shipped sidecar is custom JSON-RPC, while a real MCP surface remains future work. The root contract claim `chassis.reference-python-non-canonical` is enforced by the same boundary: `reference/python-cli` is semantic reference material, not a canonical implementation surface. The root contract claim `chassis.exports-not-policy-engines` follows the same positioning rule: Chassis may export facts to OPA, Cedar, EventCatalog-style catalogs, or similar systems, but it does not implement a competing policy language or enforcement engine.
